...
* **Comprehensive AI Assistant Support** — Native quota tracking and health awareness across 5 major AI coding companions:
  ...
  * **Ollama Cloud**: detected automatically once Ollama has run on this Mac. Requests to ollama.com are signed with `~/.ollama/id_ed25519` exactly like the Ollama desktop app does, showing the account, plan and monthly usage; no cookies, just `ollama signin`.
  * **Kimi Code / GLM Coding Plan**: monitored through [ccpm](https://github.com/chang1o/claude-code-profile-manager) profiles. Kimi shows weekly requests and the 5-hour rate limit, GLM shows 5-hour / weekly token quotas; a ccpm ollama profile with an API key shows up as an additional Ollama Cloud account.
* **ccpm multi-account** — auto-discovers ccpm profiles: Codex / Claude OAuth profiles read their own credentials, Kimi / GLM keys come from the ccpm keystore; accounts of the same service are grouped in the popover, one full card each, local cost attributed per profile directory (the primary card only counts `~/.claude` / `~/.codex`), with Dashboard / Status / Terminal / Set default actions; ccpm's `statusline` reads the cc-bar cache.
...
| **Ollama Cloud** | `~/.ollama/id_ed25519` | **Strictly Read-Only** | Signs `"<METHOD>,<PATH>?ts=<unix>"` with the ed25519 key and calls `ollama.com/api/me` and `/api/usage`, the same scheme the Ollama desktop app uses. The key never leaves the machine; no browser cookies are read. |
